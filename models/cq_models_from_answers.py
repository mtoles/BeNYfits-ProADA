...
class GPTClarifyingQuestionModel(Clarifying_Question_Model):
    ...
    #     """
    #     lm_input = f"Context: {document}\n\nTask:{task}\n\n? You are trying to complete the task but do not have enough information from the document. Ask a question about the situation that can help you complete the task. Only ask for one fact at a time. If you can, reference something specific in the document. Do not merely rephrase the fullinal task. Return the question itself, exactly, in JSON format, as in {{'question': 'The question.'}}"
...
def df_to_md_grouped(
    df: pd.DataFrame,
    output_path: str,
    groupby: List = ["doc_full", "doc_summ", "prompt"],
):
    """
...
if __name__ == "__main__":
    ...
    if not os.path.exists("results/cq/llama2-13b_20_cq_fromanswer_sample.jsonl"):
        answers = df["doc_full"].map(lambda x: x.split("."))
        ...
        df.to_json(
            "results/cq/llama2-13b_20_cq_fromanswer_sample.jsonl",
            orient="records",
            lines=True,
        )
    else:
        df = pd.read_json(
            "results/cq/llama2-13b_20_cq_fromanswer_sample.jsonl", lines=True
        )
    ...
    df.to_json(
        "results/cq/llama2-13b_20_cq_fromanswer.jsonl", orient="records", lines=True
    )

    # save results
    df_to_md(df, "results/cq/llama2-13b_20_cq_new_fromanswer.md")

    short_df = df[["doc_full", "doc_summ", "prompt", "answer", "cq"]]
