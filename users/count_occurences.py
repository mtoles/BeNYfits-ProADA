...
def main():
    user_features_file = 'user_features.py'
    benefits_programs_file = 'benefits_programs.py'
    ...
    benefits_classes = get_classes_and_bodies_from_benefits_programs(benefits_programs_file)
    ...
    references = {
        ...
        for class_name in user_features_classes
    }
    ...
    sorted_references = sorted_references[:20]  # top 10

    #    x-axis = user_features class, y-axis = referencing classes count
    class_names = [item[0] for item in sorted_references]  # uf_class
    counts = [item[1]['count'] for item in sorted_references]
    
    plt.figure(figsize=(10, 6))
    plt.bar(class_names, counts, color='skyblue')
    plt.xticks(rotation=90, ha='right')
    plt.xlabel("Class in user_features")
    plt.ylabel("Number of referencing classes in benefits_programs")
    plt.title("References from user_features classes in benefits_programs")
